...
def refresh_oauth_token(refresh_token, client_id, client_secret):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Update your storage with new access token and refresh token
            # This is a crucial step, make sure to securely store the new tokens
            return new_tokens  # Contains new access token and refresh token
        else:
            logger.error(
...
def create_zoom_meeting(meeting_conf_payload):
    ...
    response = requests.post(
        url, headers=headers, data=json.dumps(meeting_conf_payload)
    )
...
def calculate_end_time(start_time, duration):
    ...
    return end_time_obj.strftime("%Y-%m-%dT%H:%M:%S")


def create_google_calendar_event(credentials, event_payload):
    ...
    calendar_event_payload = {
        ...
        "end": {
            "dateTime": calculate_end_time(
                zoom_meeting_info["start_time"], zoom_meeting_info["duration"]
            )
        },
